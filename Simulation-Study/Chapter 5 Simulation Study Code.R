...
################
#run for data-generating process and missingness
################
...
for (s in 1:200) {
  ...
  data_comp <- simulatecomp_data( #completes data generation
    n = 300,
    T = 5,
    beta = true_beta,
    rho = 0.5,
  ...
  data_mar <- mar_missingness(data_comp) #imposes MAR missingness
  
  fit_complete <- geepack::geeglm( #complete-GEE
    y_full ~ time * z + x,
    id = id,
    data = data_comp,
    corstr = "exchangeable"
  ...
  locf_result <- fit_locf(data_mar) #LOCF
  wgee_result <- fit_wgee(data_mar) #WGEE
  mice_result <- fit_mice(data_mar, m = 20, maxit = 10, seed = 1000 + s) #MICE
  ...
  drgee_result <- fit_drgee(data_mar) #DR-GEE
  
  #formats the results properly 
  ...
  cat("Completed simulation", s, "of", 200, "\n") #outputs which simulation hsa just been completed (easier to determine run time)
}
...
#creates a summary of method performance measures (much easier to interpret)
...
p_missing_visit <- ggplot(missing_by_visit, aes(x = factor(visit), y = prop_missing)) +
  geom_col(width = 0.65, fill = "darkcyan") +
  geom_text(
...
ggsave(
  "C:/Users/curti/Downloads/missingness_by_visitSIM.png",
  width = 8,
  height = 6,
  dpi = 300
)
...
ggsave(
  "C:/Users/curti/Downloads/missingness_heatmapSIM.png",
  width = 8,
  height = 6,
  dpi = 300
)
